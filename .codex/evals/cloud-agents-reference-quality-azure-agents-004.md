## EVAL DEFINITION: cloud-agents-reference-quality-azure-agents-004

### Scope
Exactly five Azure agents in stable sorted order after validated batch 003:
1. agents/azure/azure-live-app-service-slot-swap-guard-agent
2. agents/azure/azure-live-arm-deployment-stack-guard-agent
3. agents/azure/azure-live-cost-budget-action-guard-agent
4. agents/azure/azure-live-entra-role-assignment-guard-agent
5. agents/azure/azure-live-keyvault-rotation-purge-guard-agent

### AgentCore reference standard
Agent-local references must match the stronger AgentCore-style pattern: component-specific operations guide with version note, what people get wrong, officially grounded service shape, key insight, non-negotiable rules, minimal safe flow, high-risk assumptions, safe verification targets, and push-back rules. Common references must cover official sources, safety checklist, evidence labels, workflow/output contract, and generic documentation MCP wording.

### Microsoft Learn evidence used
- Azure App Service deployment slots: swap phases, swap with preview, sticky and non-swapped settings, warmup settings, rollback by re-swap, and limitations.
- Azure Deployment Stacks and ARM/Bicep deployment safety: managed resources, action-on-unmanage, deny settings, limitations, and stack out-of-sync warning handling.
- Azure Cost Management and Quotas: budget alerts, actual and forecasted thresholds, cost-data latency, alert behavior, quota monitoring, and GPU cost-risk guidance.
- Azure RBAC and Microsoft Entra PIM: least privilege, privileged administrator role limits, group-based assignment preference, eligible versus active assignments, and PIM conditions.
- Azure Key Vault: soft delete retention, purge protection, purge privileges, recovery limitations, key rotation policy, versioning, and dependent-service timing.

### Capability Evals
- [x] Each target has one component-specific operations guide with AgentCore-standard headings and actionable, source-grounded content.
- [x] Each target has refreshed official-sources, safety-checklist, workflow-and-output, and mcp-and-evidence references.
- [x] Each target bumps patch version in AGENT.md, metadata.json, and catalog/agents.json.
- [x] Existing stale Azure tool phrasing is replaced with approved generic documentation evidence phrasing.
- [x] Agent harnesses remain compact and point to the bound skill/reference discipline without leaking environment-specific identifiers.
- [x] Live adjacent permission/preflight/rollback docs avoid environment-specific IDs and require explicit approval before mutations.

### Regression Evals
- [x] Structural AgentCore heading grader passes.
- [x] Prohibited wording grep passes.
- [x] AWS scoped diff check returns no paths.
- [x] npm run validate:agent-schema passes.
- [x] npm run validate:plugin-manifest passes after plugin manifest regeneration.
- [x] npm run validate:kiro-powers passes after Kiro Powers regeneration.
- [x] npm run validate:multi-harness-marketplace passes.
- [x] npm run validate:codex-marketplace passes.
- [x] npm run validate:asset-integrity passes after integrity regeneration.
- [x] npm run validate passes.

### Verdict
Pass. Full `npm run validate` completed successfully with `VALIDATE_EXIT:0`; Azure agents batch 004 is validated and remains uncommitted.
