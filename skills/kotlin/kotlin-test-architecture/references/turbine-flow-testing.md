...
- Turbine's `test { }` extension on a Flow collects emissions inside a coroutine and provides `awaitItem()`/`awaitComplete()`/`awaitError()` assertions with a configurable timeout, replacing manual `toList()`/`first()` collection in tests.
