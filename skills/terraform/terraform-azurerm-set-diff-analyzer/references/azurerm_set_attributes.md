...
### Service-domain cross-checks

When the resource is `azurerm_application_gateway`, cross-check the nested block you are modeling against the Azure Application Gateway routing model:
...
When the resource is `azurerm_network_security_group`, cross-check the nested block against Azure NSG rule semantics:

- priority,
- direction,
- access,
- protocol,
- address and port scope.
...
- Azure service docs are useful to confirm runtime meaning, but Terraform provider docs remain the source of truth for the nested block names being modeled here
