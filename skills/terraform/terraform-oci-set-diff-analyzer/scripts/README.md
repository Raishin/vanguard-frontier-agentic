# Terraform OCI Set Diff Analyzer Script

A Python script that analyzes Terraform plan JSON and identifies "false-positive diffs" in AzureRM Set-type attributes.

## Overview

OCI Terraform Provider's Set-type attributes (such as `backend_address_pool`, `security_rule`, etc.) don't guarantee order, so when adding or removing elements, all elements appear as "changed". This script distinguishes such "false-positive diffs" from actual changes.
...
By default, uses `references/oci_set_attributes.json`, but you can specify a custom definition file:

```bash
python analyze_plan.py plan.json --attributes /path/to/custom_attributes.json
```

See `references/oci_set_attributes.md` for the definition file format.

## Limitations

- Only OCI resources (`azurerm_*`) are supported
...
- [oci_set_attributes.md](../references/oci_set_attributes.md) - Attribute definition reference
