...
# -*- coding: utf-8 -*-
"""
Terraform Plan Analyzer for OCI Set-type Attributes
...
DEFAULT_ATTRIBUTES_PATH = (
    Path(__file__).parent.parent / "references" / "oci_set_attributes.json"
)
...
OCI_SET_ATTRIBUTES: Dict[str, Any] = {}
...
def analyze_resource_change(
    ...
    # Skip if no change or not an OCI resource
    if actions == ["no-op"] or not resource_type.startswith("oci_"):
        return None
    ...
    set_attrs = OCI_SET_ATTRIBUTES.get(resource_type, {})
...
def format_markdown_output(result: AnalysisResult) -> str:
    ...
    lines.append(
        'Analyzes OCI Set-type attribute changes and identifies order-only "false-positive diffs".'
    )
    lines.append("")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Analyze Terraform plan JSON for OCI Set-type attribute changes.",
...
def main():
    ...
    OCI_SET_ATTRIBUTES = load_set_attributes(args.attributes)
