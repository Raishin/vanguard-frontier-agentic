...
- Recommended before review. Fetch current MLflow 3 GenAI Tracing and Evaluation documentation to confirm judge names, API signatures, and the current status of trace-storage variants (experiment versus Delta table).
- Verify the current built-in judge count and names; this list is definitive and not to be guessed from memory.
