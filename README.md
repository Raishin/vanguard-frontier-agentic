...
.NET, Java, Kotlin, PHP, Python, TypeScript, FinOps, Accounting, Finance, Oracle NetSuite ERP, SAP S/4HANA + BTP ERP, Microsoft 365 / Dynamics 365, a cloud-neutral Databricks lakehouse/AI board, and Azure-ecosystem Snowflake data-platform**
agentic ecosystems.
...
- **Bundled:** all <!-- count:agents -->697<!-- /count --> cloud, security, compliance, Kubernetes, Terraform agents (incl. provider maestros and live-guard agents)
...
- **Bundled:** <!-- count:agents -->697<!-- /count --> Copilot agent adapters under `agents/<provider>/<agent>/harnesses/copilot.agent.md`
...
- **Plugin manifest:** [`.cursor-plugin/plugin.json`](.cursor-plugin/plugin.json) enumerates all **<!-- count:agents -->697<!-- /count --> Cursor agent adapters** explicitly via the `agents` field
...
- **For agent adapter files** (`.codex/agents/*.toml`): after enabling the plugin, run `npx vfa-export-agents --platform codex --all --repo .` to write the <!-- count:agents -->697<!-- /count --> agent adapters into your repo
...
**<!-- count:skills -->720<!-- /count --> skills** across AWS, Azure, OCI, GCP, Alibaba Cloud, Huawei Cloud, Kubernetes, CNCF ecosystem, Terraform, marketing governance, and more.
...
| 🧱 Databricks        |    20 | Cloud-neutral board (17): maestro routing + platform architecture, Unity Catalog governance, identity/network security, data protection & privacy, Lakeflow pipelines, streaming reliability, data quality & observability, SQL performance, AI/BI Genie, MLOps, GenAI agent engineering, GenAI evaluation & observability, developer platform (bundles/CI-CD), platform reliability, FinOps cost, value realization — static review; plus Azure-specific (3): Unity Catalog governance + lakehouse engineering (`-at-azure`) and a Unity Catalog schema-scoped grant guard (mutating-runtime live-guard, Phase B) |
...
**<!-- count:agents -->697<!-- /count --> agents** matching the skill catalog — agents ship harness adapters and a hardened permission model.
...
Current catalog: **<!-- count:agents -->697<!-- /count --> agents · <!-- count:skills -->720<!-- /count --> skills · <!-- count:providers -->45<!-- /count --> cloud/platform providers**
