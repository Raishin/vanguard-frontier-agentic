...
The GPT-6 and GPT-5.6 families advertise a `max` reasoning effort on the OpenAI models API, so `max` is now part of the codex harness vocabulary and appears in the rows for those models. Earlier revisions of this table withheld `max` pending confirmation that the Codex CLI's `ReasoningEffort` enum accepted it; that exclusion has been lifted on the strength of the models API listing, and an operator who sees Codex reject the value should re-open the question here rather than in the policy file. `gpt-6-astra` is the exception within that group: it does not advertise `none`, so that value is omitted from its row.
...
| codex → OpenRouter | HTTP 404 on an unrecognized slug | Reasoning-effort passthrough from Codex to a custom provider is undocumented; the registry gives this namespace an empty `reasoning_efforts` list, so the policy engine fails closed rather than silently dropping the field |
