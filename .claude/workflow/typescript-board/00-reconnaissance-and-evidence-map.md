...
| Maestro routing fixture | `tests/fixtures/<provider>-maestro-routing/` with `taxonomy.json` + `inputs/` + `expected/`; **all three** generated by `tests/_generate_maestro_routing_fixtures.py` (discovers providers by their `*-maestro` skill directory; `:308` overwrites `taxonomy.json` from agent ids and summaries on every run) | fixture required for the board's maestro. Since implementation the miner is a *fallback*: an agent that declares `routing_keywords` in its `metadata.json` has those ranked first, so the primary lever is the declared vocabulary and **summary wording is the secondary one** |
| Generated `live_guard_intent` | `GATE_INTENT["default"]` in `tests/_generate_maestro_routing_fixtures.py`; java and php carry `(destroy\|delete\|terminate\|rollout to prod\|…)`. The dict is keyed by provider and its comment promised a per-provider override, but `build_taxonomy()` read `["default"]` unconditionally — the override was wired during implementation. hr, legal, and netsuite ship **no such key at all**, and this board joins them | the key is optional; a gate regex black-holes any domain whose vocabulary it matches, whether a domain noun or a bare verb that is ordinary board language (see [04 §5.4](./04-routing-architecture-and-fixtures.md)) |
