...
Taxonomy: 3 domains × (24/25/19 keywords), parallel_threshold=0.6, live_guards=[]
...
All 6 skills evaluated on clarity, FOCUS compliance, least-privilege, security posture, provenance labeling:
  ...
  rightsize-recommendation:       5/5 — PASS (WebFetch absent ✓, headroom formula ✓, Karpenter criteria ✓)
  carbon-cost-pair:              5/5 — PASS (Scope 2 market-based ✓, CSRD/SEC ✓, kgCO2e ✓)
  focus-spec-normalizer:         5/5 — PASS (WebFetch absent ✓, all 4 vendor adapters ✓, FOCUS v1.2 columns ✓)
...
1. Security posture is consistently strong — unconditional credential refusal with per-format enumeration in companion docs
2. Provenance labeling (live-price / documentation-based / assumed / excluded) is first-class in every data-producing skill
3. WebFetch boundary correctly drawn: rightsize-recommendation and focus-spec-normalizer correctly exclude it; live-price skills include it with rationale
4. FOCUS v1.2 column coverage is accurate; gaps are documented with null handling and resolution notes
5. Maestro live_guards=[] is correct for v1 — no mutating specialists wired
