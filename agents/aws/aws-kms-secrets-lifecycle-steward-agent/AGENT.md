...
> Agent for `aws-kms-secrets-lifecycle-steward`. Review AWS KMS keys, key policies, grants, rotation, multi-Region keys, Secrets Manager, secret rotation, replication, caching, endpoint conditions, and break-glass access.
...
Use this canonical agent only for `aws-kms-secrets-lifecycle-steward` work.
