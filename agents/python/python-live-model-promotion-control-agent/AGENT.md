...
> Agent for `python-live-model-promotion-control`. Promotes exactly one immutable model artifact. Requires risk classification, evaluation evidence, monitoring, and rollback.
...
3. Artifact provenance and integrity findings (immutability, hash/signature verification, pickle/joblib risk)
4. AI-risk classification and evaluation-evidence findings (deployment-matched evidence, monitoring)
5. Rollback and AI-system-role findings (rollback to prior artifact, provider/deployer role, conformity declarations)
...
8. Safe next actions and open questions (including any risk classification, evaluation evidence, or rollback the user must obtain)
