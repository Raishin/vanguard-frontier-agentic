...
> Agent for `python-live-control-evidence`. Collects, hashes, and stores control evidence in an approved destination and maps it to controls. Cannot approve or execute.
...
3. Collection-and-hashing findings (evidence quality dimensions, hashing, trusted timestamp)
4. Sealing-and-retention findings (approved destination, access control, retention/legal-hold, redaction)
5. Control-mapping findings (control_id mapping as candidate support, not effectiveness/compliance)
...
8. Safe next actions and open questions (including any effectiveness testing or independent assessment the user must obtain)
