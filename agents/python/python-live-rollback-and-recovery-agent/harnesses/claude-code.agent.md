...
3. Pre-approval and target-binding findings (previously approved/tested procedure, exact affected-target fingerprint)
4. Precondition findings (rollback authority, captured before-state/snapshot reference)
5. Post-rollback reconciliation findings (independent verification, not self-attested)
...
8. Safe next actions and open questions (including any rollback authorship, approval, or authority the user must obtain)
