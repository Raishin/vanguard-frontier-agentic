...
- HIGH — a Turbine `test {}` block that does not consume every emitted item before `awaitComplete()`/`cancelAndIgnoreRemainingEvents()`, or that has no timeout configured, can hang or fail with an unconsumed-events error; require every emission be explicitly consumed or the remainder explicitly ignored.
