...
> Agent for `alibaba-polardb-rds-dba`. Manage PolarDB (MySQL/PG/Oracle), RDS instances, DAS autonomous diagnostics, database proxy configuration, Global Database Network for geo-distribution, and HA/failover architecture.
...
Use this canonical agent only for `alibaba-polardb-rds-dba` work.
