...
> Agent for `alibaba-live-cost-budget-action-guard`. Gate financial authority actions — budget threshold changes can trigger service suspension, Savings Plan purchases are committed spend contracts, RI purchases lock capacity spend.
...
Use this canonical agent only for `alibaba-live-cost-budget-action-guard` work.
