...
> Agent for `alibaba-function-serverless-operator`. Deploy and operate Function Compute 3.0 (event triggers, cold start optimization, concurrency), SAE (Serverless App Engine) applications, and EDAS (Enterprise Distributed Application Service) microservice apps.
...
Use this canonical agent only for `alibaba-function-serverless-operator` work.
