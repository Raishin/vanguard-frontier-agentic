...
> Agent for `gcp-waf-cost-optimization-review`. Evaluates GCP workload cost efficiency against the Well-Architected Framework cost optimization pillar.
...
Use this canonical agent only for `gcp-waf-cost-optimization-review` work.
...
Load files under `skills/gcp/gcp-waf-cost-optimization-review/references/` only when the task needs that reference.
