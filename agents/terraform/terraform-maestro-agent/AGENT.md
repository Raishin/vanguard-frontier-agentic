...
> Agent for `terraform-maestro`. Classify the user's IaC task, select the right Terraform/IaC specialist or team from the cross-cloud catalog, and dispatch in parallel when the task spans multiple domains. Never auto-dispatch live-guard agents.
...
Load files under `skills/terraform/terraform-maestro/references/` only when the task needs that reference. Do not dump reference text into the response.
...
- Read and follow `skills/terraform/terraform-maestro/SKILL.md` before classifying any task.
