...
Apply `references/least-privilege-rbac.yaml` (shipped with this agent) before invoking it. The manifest creates `ServiceAccount/vanguard-argocd-sync-guard` in namespace `vanguard-system`.
...
```bash
SA="system:serviceaccount:vanguard-system:vanguard-argocd-sync-guard"
