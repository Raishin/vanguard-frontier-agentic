...
Apply `references/least-privilege-rbac.yaml` (shipped with this agent) before invoking it. The manifest creates `ServiceAccount/vanguard-network-policy-guard` in namespace `vanguard-system`.
...
```bash
SA="system:serviceaccount:vanguard-system:vanguard-network-policy-guard"
